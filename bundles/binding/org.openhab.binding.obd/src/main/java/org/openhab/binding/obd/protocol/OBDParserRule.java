...
public class OBDParserRule {
	...
	public OBDParserRule(String rule) throws OBDException {
		try {
			...
			if (!matcher.matches()) {
				throw new OBDException("Invalid parser rule '" + rule + "', given data type '" + parts[1] + "' does not follow the expected pattern '<DataType>(<pattern>)'");
			}
			...
			try {
				...
				throw new OBDException("Invalid parser rule '" + rule + "', unknown data type");
			}
			...
			if (b.length > 8) {
				throw new OBDException("Invalid parser rule '" + rule + "', too many bytes");
			}
			...
			throw new OBDException("Invalid parser rule", e);
		}
	}
