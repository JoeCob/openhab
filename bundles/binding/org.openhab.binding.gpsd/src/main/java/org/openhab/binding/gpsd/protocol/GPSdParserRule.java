...
public class GPSdParserRule {
	...
	public GPSdParserRule(String rule) throws GPSdException {
		try {
			
			String[] parts = rule.split(":");
			
			/*if(parts.length != 2) {
				throw new GPSdException("Invalid parser rule '" + rule + "'");
			}*/
			...
			if (!matcher.matches()) {
				throw new GPSdException("Invalid parser rule '" + rule + "', given data type '" + parts[1] + "' does not follow the expected pattern '<DataType>(<pattern>)'");
			}
			...
			try {
				...
				throw new GPSdException("Invalid parser rule '" + rule + "', unknown data type");
			}
			...
			if (b.length > 8) {
				throw new GPSdException("Invalid parser rule '" + rule + "', too many bytes");
			}
			...
			throw new GPSdException("Invalid parser rule", e);
		}
	}
