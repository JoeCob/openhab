...
public class ResultParser extends AbstractResultParser {
	...
	/**
	 * parse {@link JSONObject, String line} into {@link IGPSObject}
	 * 
	 * @param json
	 *            the {@link JSONObject} to parse
	 * @param line
	 *  		  original device sentence to be passed over.           
	...
	@Override
	public IGPSObject parse(final JSONObject json, String line) throws ParseException {
		IGPSObject gps = null;
		final String clazz = json.optString("class");
		
		if (TPVObject.NAME.equals(clazz)) {
			gps = this.parseTPV(json, line);
		} else 
		{ 
			gps = this.parse(json);
		}
	...
	private IGPSObject parseTPV(final JSONObject json) {
		IGPSObject gps;
		gps = parseTPV(json, null);
		return gps;
	}
	
	private IGPSObject parseTPV(final JSONObject json, String line) {
		IGPSObject gps;
